...
BEGIN
    UPDATE MOVILES.MATRICULA
    SET NOTA = P_NOTA
    WHERE GRUPO_ID = P_GRUPO_ID AND CEDULA_ALUMNO = P_ALUMNO_ID;
END;
/

create OR REPLACE FUNCTION MOVILES.OBTENER_MATRICULAS_POR_GRUPO(
    P_GRUPO_ID IN VARCHAR2
) RETURN SYS_REFCURSOR AS
    RESULT_CURSOR SYS_REFCURSOR;
BEGIN
    OPEN RESULT_CURSOR FOR SELECT * FROM MOVILES.MATRICULA WHERE GRUPO_ID = P_GRUPO_ID;
